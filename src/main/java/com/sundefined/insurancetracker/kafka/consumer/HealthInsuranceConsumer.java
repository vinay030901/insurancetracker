...
@Slf4j
public class HealthInsuranceConsumer {

    private final PolicyEventRepository repository;

    @KafkaListener(topics = "policy-health-events", groupId = "health-consumer-group")
    public void consume(PolicyEvent event) {
        log.info("[Kafka][HealthConsumer] Received event for policyId={} at stage={}",
                event.getPolicyId(), event.getStage());
        repository.save(event);
        log.info("[Kafka][HealthConsumer] Saved event for policyId={} at stage={}",
                event.getPolicyId(), event.getStage());
    }
}
